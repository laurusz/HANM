...
def HANM_V(data):
    ...
    d3 = h1
    # decoder
    decoder1 = Dense(d1, activation='relu')(z)  # 1-15-7-15-N
    decoder2 = Dense(d2, activation='relu')(decoder1)
    decoder3 = Dense(d3, activation='relu')(decoder2)
    x_hat = Dense(original_dim, activation='sigmoid')(decoder3)
    ...
    # encode
    encoder1 = Model(x, z_mean)
    encoder2 = Model(x, z_log_var)
    x_test_encoded1 = encoder1.predict(x_train, batch_size=batch_size)
    x_test_encoded2 = encoder2.predict(x_train, batch_size=batch_size)
    z = Lambda(sampling, output_shape=(latent_dim,))([x_test_encoded1, x_test_encoded2])
    z = K.eval(z)
    return z
...
def HANM(data, seed=0):
    ...
    **Description**: Hierarchical Additive Noise Model (HANM) generalizes 
    many-to-one causality into an approximate pair relationship through
    ...
    **Data Type**: Continuous
    ...
    for i in range(0, D - 1):
        ...
        ny = ny.reshape(-1, 1)
        y_to_x = HSIC(y, ny)
        delta = 0.1 * min(x_to_y, y_to_x)
        if x_to_y + delta < y_to_x:
            # print('result:x' + str(i) + '->y', 'x->y:', x_to_y, ' y->x' + str(i) + ':', y_to_x)
            result[i] = 1
        elif y_to_x + delta < x_to_y:
            # print('result:y->x' + str(i), 'x->y:', x_to_y, ' y->x' + str(i) + ':', y_to_x)
            result[i] = 0
        else:
            result[i] = -1
            # print('x->y:', x_to_y, ' y->x' + str(i) + ':', y_to_x)
    return result
